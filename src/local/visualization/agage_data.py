...
def plot_station_locations(files, title, save_file_name):
    ...
    for file in files:
        ...
        stations.append({"name": file.stem, "lat": lat, "lon": lon})
    ...
    plt.title(title)
    plt.savefig(plots_path / f"{save_file_name}.png", dpi=300, bbox_inches="tight")
    plt.close()
...
plot_station_locations(
    some_files, title="AGAGE station locations", save_file_name="AGAGE_locations"
)
